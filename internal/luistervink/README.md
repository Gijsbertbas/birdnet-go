...
The `luistervink` package provides functionality for integrating with the Luistervink API service. This package handles uploading audio files and detection data to the Luistervink platform for bird detection and soundscape sharing.

## Overview

Luistervink is a service that allows sharing of bird detections and environmental audio data. This package enables automated submission of audio recordings and bird species detections to the Luistervink platform.
...
- **API Integration**: Connect to the Luistervink API for data submission
...
- **Connection Testing**: Diagnostic tools to test connectivity with the Luistervink service
...
The package provides a client interface for interacting with the Luistervink API:
...
`BwClient` is the main implementation that provides methods to interact with the Luistervink API:
...
> **Note**: Currently, Luistervink ignores user-submitted coordinates for BirdNET-Pi/Go implementations. The service always uses coordinates assigned to the station's token/ID. The location randomization feature is implemented but unused at this time.

### Connection Testing

Comprehensive testing tools to verify connectivity with Luistervink services:
...
func initLuistervink(settings *conf.Settings) (*luistervink.BwClient, error) {
    // settings should contain:
    // - Realtime.Luistervink.ID: "your-station-id"
    // - Realtime.Luistervink.LocationAccuracy: 1000 (meters)
    // - Realtime.Audio.FfmpegPath: "/path/to/ffmpeg"
    // - BirdNET.Latitude: 60.1234
    // - BirdNET.Longitude: 24.5678

    client, err := luistervink.New(settings)
    if err != nil {
        return nil, fmt.Errorf("failed to create Luistervink client: %w", err)
    }
    return client, nil
}
...
func publishDetection(client *luistervink.BwClient, pcmData []byte) error {
    note := &datastore.Note{
        Date:          "2023-04-10",
        Time:          "14:30:25",
...
```go
func testConnection(client *luistervink.BwClient) {
...
When debug mode is enabled (`Settings.Realtime.Luistervink.Debug = true`), the following debug artifacts are saved:

- **Raw PCM Data**: Saves the original PCM audio data before any processing
  - Stored in `debug/luistervink/pcm/` directory
...
- **Audio Files**: Saves the processed FLAC or WAV audio files
  - Stored in `debug/luistervink/flac/` or `debug/luistervink/wav/` directory
...
- **Location coordinates** are currently ignored by Luistervink's API. Despite the location randomization feature, Luistervink always uses the coordinates assigned to your station ID/token rather than the coordinates submitted with detections.
